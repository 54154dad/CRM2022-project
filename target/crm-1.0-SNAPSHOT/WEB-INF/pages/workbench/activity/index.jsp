	...
	<script type="text/javascript">
		...
		$(function(){
			...
			$("#tBody").on("click","input[type='checkbox']",function () {
				//如果列表中的所有的checkbox都选中，则全选按钮也选中
				if ($("#tBody input[type='checkbox']").size() == $("#tBody input[type='checkbox']:checked").size()){
					$("#allClick").prop("checked",true);
				}else {//如果列表中的所有checkbox中至少有一个未选中，则全选按钮也取消
			...
			$("#exportActivityXzBtn").click(function () {
				...
				//发生请求
				//发送同步请求
				window.location.href='workbench/activity/queryAllActivitiesById.do?'+ids;
				//发送请求完后，把复选框取消选中状态
			...
			$("#updateById").click(function () {
				//接收复选框的数据
				var checkdId = $("#tBody input[type='checkbox']:checked");
				if (checkdId.size() == 0){
					alert("请选择要修改的市场活动!");
					return;
				}
		...
		function queryActivityByConditionForPage(pageNo,pageSize) {
			...
			$.ajax({
				...
				success:function (data) {
					...
					//对容器调用bs_pagination函数，显示翻页信息
					$("#demo_pag1").bs_pagination({
						currentPage:pageNo,//当前页号，相当与pageNo
		...
		第二种方法：
			$(document).ready(function(){
				$("[name='allClick']").click(function(){
					if($(this).is(':checked')){
						$("[name='oneCheck']").attr('checked',true);
					}else{
						$("[name='oneCheck']").attr('checked',false);
					}
				})
				$("[name='oneCheck']").click(function(){
					var allClick = false;
					...
					if(allClick){
						$("[name='allClick']").attr('checked',false);
					}else{
						$("[name='allClick']").attr('checked',true);
					}
				})
			});*/
		...
		第三种方法：
			function allClick() {
				//获得当前点击后全选按钮的状态
				var flag = $("#allClick").is(":checked");  //prop方法也可以
	...
	<div style="width: 100%; position: absolute;top: 5px; left: 10px;">
		...
		<div class="btn-toolbar" role="toolbar" style="background-color: #F7F7F7; height: 50px; position: relative;top: 5px;">
			<div class="btn-group" style="position: relative; top: 18%;">
				...
				<button type="button" class="btn btn-default" data-toggle="modal" data-target="#editActivityModal" id="updateById"><span class="glyphicon glyphicon-pencil"></span> 修改</button>
		...
		<div style="position: relative;top: 10px;">
			<table class="table table-hover">
				...
				<tr style="color: #B3B3B3;">
					<td><input type="checkbox" id="allClick"/></td>
					<td>名称</td>
					<td>所有者</td>
					<td>开始日期</td>
					<td>结束日期</td>
